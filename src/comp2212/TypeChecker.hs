...
typeOfConstruct (IfThenElse exp c1 c2, rest, tenv)
    ...
    | otherwise = error ("if-then-else construct expects a boolean expression. " ++ (unparse exp) ++ "does not evaluate to a boolean") where
        new_rest = formatConstruct c1 ++ formatConstruct c2 ++ rest

typeOfConstruct (IfThen exp c, rest, tenv)
    | typeOfExp exp tenv == SplBool = typeOfConstruct (fromJust $ safeHead new_rest, safeTail new_rest, tenv)
    | otherwise = error ("if construct expects a boolean expression. " ++ (unparse exp) ++ " does not evaluate to a boolean") where
        new_rest = formatConstruct c ++ rest

typeOfConstruct (While exp c, rest, tenv)
    | typeOfExp exp tenv == SplBool = typeOfConstruct (fromJust $ safeHead new_rest, safeTail new_rest, tenv)
    | otherwise = error ("While construct expects a boolean expression. " ++ (unparse exp) ++ " does not evaluate to a boolean") where
        new_rest = formatConstruct c ++ rest
...
typeOfConstruct (VarAssign var exp, rest, tenv)
    ...
    | otherwise = error (quote var ++ " expects type " ++ show varType ++ ", but" ++ show expType ++ " was supplied to it") where
        varType = lookUpType var tenv
        expType = typeOfExp exp tenv

typeOfConstruct (StackOperationAssign var1 (Push var2 exp), rest, tenv)
    | var1Type == SplSingleList = if var2Type == SplSingleList && expType == SplInt then newState else error (quote var1 ++ " expects an integer")
    | var1Type == SplDoubleList = if var2Type == SplDoubleList && expType == SplSingleList then newState else error (quote var1 ++ " expects a single list") where
...
typeOfConstruct (StackOperationAssign var1 (Pop var2), rest, tenv)
    | lookUpType var1 tenv == SplInt = if lookUpType var2 tenv == SplSingleList then newState else error (quote var1 ++ " expects an integer.")
    | lookUpType var1 tenv == SplSingleList = if lookUpType var2 tenv == SplDoubleList then newState else error (quote var1 ++ " expects a single list")
...
typeOfConstruct (IntDeclareAssignExp var exp, rest, tenv)
    ...
    | otherwise = error (quote var ++ " expects type SplInt, but " ++ show expType ++ " was supplied to it") where
        expType = typeOfExp exp tenv
...
typeOfConstruct (IntDeclareAssignPop var list, rest, tenv)
    ...
    | otherwise = error (quote var ++ " expects type SplInt, but " ++ show listType ++ " was supplied to it") where
...
typeOfConstruct (BoolDeclareAssign var exp, rest, tenv)
    ...
    | otherwise = error (quote var ++ " expects type SplBool, but " ++ show expType ++ " was supplied to it") where
        expType = typeOfExp exp tenv
...
typeOfConstruct (SingleListDeclareAssignPop var doubleList, rest, tenv)
    ...
    | otherwise = error (quote var ++ " expects type SplSingleList, but " ++ show expType ++ " was supplied to it") where
...
typeOfConstruct (SingleStackOperation (Push var exp), rest, tenv)
    ...
    | otherwise = error (quote var ++ " is not of a list type. Push operation must be done on a list type") where
        newState = typeOfConstruct (fromJust $ safeHead rest, safeTail rest, tenv) 
        expType = typeOfExp exp tenv
...
typeOfConstruct (SingleStackOperation (Pop var), rest, tenv)
    ...
    | otherwise = error ("Unable to pop from " ++ quote var ++ ". Pop operation not supported for " ++ (show $ lookUpType var tenv))
...
typeOfConstruct (Sort var, rest, tenv)
    ...
    | otherwise = error (quote var ++ " is not of type SplSingleList. Sort operation not supported for " ++ (show $ lookUpType var tenv))
...
typeOfExp (Length var) tenv
    ...
    | otherwise = error ("length must be called on a list type. " ++ quote var ++ " is not a list type")

typeOfExp (Empty var) tenv
    | lookUpType var tenv == SplSingleList || lookUpType var tenv == SplDoubleList = SplBool
    | otherwise = error ("empty must be called on a list type. " ++ quote var ++ " is not a list type")
...
typeOfExp (Modulo e1 e2) tenv = checkArithmetic "%" e1 e2 tenv
...
typeOfExp (Not e) tenv
    ...
    | otherwise = error ("not operator expects a boolean. " ++ show (typeOfExp e tenv) ++ " was provided")
...
checkArithmetic operator e1 e2 tenv
    ...
    | typeOfExp e1 tenv /= SplInt = error (operator ++ " expects two integers. " ++ (unparse e1) ++ " does not evaluate to an interger")
    | typeOfExp e2 tenv /= SplInt = error (operator ++ " expects two integers. " ++ (unparse e2) ++ " does not evaluate to an integer")
    | otherwise = error (operator ++ " expects two integers. " ++ (unparse e1) ++ " and " ++ (unparse e2) ++ " do not evaluate to integers")
...
checkComparator operator e1 e2 tenv
    ...
    | typeOfExp e1 tenv /= SplInt = error (operator ++ " expects two integer. " ++ (unparse e1) ++ " does not evaluate to an integer")
    | typeOfExp e2 tenv /= SplInt = error (operator ++ " expects two integer. " ++ (unparse e2) ++ " does not evaluate to an integer")
    | otherwise = error (operator ++ " expects two integers. " ++ (unparse e1) ++ " and " ++ (unparse e2) ++ " do not evaluate to integers")

lookUpType :: String -> TypeEnvironment -> Types
lookUpType x [] = error ("Variable " ++ quote x ++ " not declared")
...
unparse :: Exp -> String
unparse (Int x) = quote $ show x
unparse (List xs) = quote xs
unparse (String xs) = quote [ x | x <- xs, x /= '\"']
unparse (Sequences xss) = quote xss
unparse BoolTrue = quote "true"
unparse BoolFalse = quote "false"
unparse (Add x y) = unparse x ++ "+" ++ unparse y
unparse (Minus x y) = unparse x ++ "-" ++ unparse y
unparse (Multiply x y) = unparse x ++ "*" ++ unparse y
unparse (Divide x y) = unparse x ++ "/" ++ unparse y
unparse (LessThan x y) = unparse x ++ "<" ++ unparse y
unparse (GreaterThan x y) = unparse x ++ ">" ++ unparse y
unparse (EqualTo x y) = unparse x ++ "=" ++ unparse y
unparse (LTE x y) = unparse x ++ "<=" ++ unparse y
unparse (GTE x y) = unparse x ++ ">=" ++ unparse y
unparse (Not x) = "not " ++ unparse x
unparse (Length list) = quote (show list ++ ".length")
unparse (Empty list) = quote (show list ++ ".empty")
unparse (Var string) = quote (string)
